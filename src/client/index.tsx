...
const css = [
  ...
  // Tooltip: same dark pill as the host Tooltip (black rounded label, no
  // native title balloon). The delay mirrors the host tooltip's settle; the
  // fade is short so the pill appears right beside the hovered button.
...
function favoriteButtonOf(messageId: string, lang: Record<string, string>): HTMLButtonElement {
  ...
  button.dataset.starred = isFavorite(favoritesSnapshot(), actionCtx.sessionId ?? '', messageId) ? 'true' : undefined
  button.classList.toggle('crl_on', isFavorite(favoritesSnapshot(), actionCtx.sessionId ?? '', messageId))
  // The CSS tooltip reads data-tip (a short dark pill like the host Tooltip);
  // the native title balloon is intentionally absent — double tips would
  // stack a browser bubble over the styled one.
...
function refreshFavoriteButton(button: HTMLButtonElement, lang: Record<string, string>): void {
  ...
  // Both the a11y label and the styled tooltip follow the new state.
