window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function jumpToMessage(sessionsService, sessionId, key, nodeOf, onProgress, signal) {
			...
			const items = effectiveMessages.map((m, i) => {
				...
				return (0, react.createElement)("button", {
					...
					onClick: () => {
						...
						setJumping(true);
						jumpToMessage(sessionsService, sessionId, key, (k) => chatNodeOf(fallbackStore.getSnapshot(), k), void 0, controller.signal).finally(() => setJumping(false));
					},
