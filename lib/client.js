window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function syncOfficialHide(showOfficial) {
			const el = document.head.querySelector(`style[${HIDE_OFFICIAL_STYLE_ID}]`);
			if (showOfficial) {
			...
			} else if (el.textContent !== HIDE_OFFICIAL_CSS) el.textContent = HIDE_OFFICIAL_CSS;
		}
		/**
		...
		*/
		...
		function clearRowActions() {
			for (const row of document.querySelectorAll("[data-crl-actions=\"1\"]")) {
				for (const btn of row.querySelectorAll("[data-crl-star],[data-crl-fill]")) btn.remove();
				row.dataset.crlActions = "";
			}
		}
		let railSettingsScope = null;
		/** client apply 时绑定（settingsScope 官方服务提供）。 */
		function bindRailSettingsScope(scope) {
			if (typeof scope !== "object" || scope === null) return;
			railSettingsScope = scope.bind({ namespace: "chat-rail" });
		}
		...
		if (typeof document !== "undefined") {
			const existing = document.head.querySelector(`style[data-plugin-css="${STYLE_ID}"]`);
			if (existing === null) {
				const tag = document.createElement("style");
		...
		*/
		function chatNodeOf(snapshot, key) {
			if (key === void 0 || snapshot === void 0 || snapshot === null) return void 0;
			const store = snapshot?.nodes;
			if (store !== void 0 && typeof store.get === "function") return store.get(key);
			return (snapshot?.chat?.nodes)?.get(key);
		}
		/** Map one chat node (either plane) to a rail message entry. */
		function railMessageOfNode(node) {
		...
		*  values) or the legacy Session snapshot's `chat.nodes` Map. */
		function collectFromNodes(snapshot) {
			const out = [];
			const store = snapshot?.nodes;
			if (store !== void 0 && typeof store.values === "function") for (const node of store.values()) {
				const m = railMessageOfNode(node);
				if (m !== null) out.push(m);
			}
			else {
				const chat = snapshot?.chat;
				if (chat?.nodes === void 0) return out;
		...
		function nodeValuesOf(snapshot) {
			const store = snapshot?.nodes;
			if (store !== void 0 && typeof store.values === "function") return [...store.values()];
			const chat = snapshot?.chat;
			if (chat?.nodes === void 0) return [];
		...
		*/
		function collectQaFromNodes(snapshot) {
			const out = [];
			for (const node of nodeValuesOf(snapshot)) {
				...
				if (callId === "") continue;
				if ((typeof r.name === "string" ? r.name : typeof r.call?.name === "string" ? r.call.name : "") !== "ask_user_question") continue;
				const asked = questionsTextOf(typeof r.argsRaw === "string" ? r.argsRaw : typeof r.call?.argsRaw === "string" ? r.call.argsRaw : "");
				if (asked === "") continue;
				out.push({
					seq: typeof n.anchorSeq === "number" ? n.anchorSeq : 0,
					...
					text: asked,
					hasImage: false,
					key: typeof n.key === "string" ? n.key : void 0,
					anchor: `call:${callId}`,
					qa: true
				});
			}
		...
		*/
		function qaWithinMessage(qaMessages, messages, index) {
			const self = messages[index];
			if (self === void 0) return [];
			const next = messages[index + 1];
			const upper = next === void 0 ? void 0 : next.seq;
			return qaMessages.filter((q) => q.seq >= self.seq && (upper === void 0 || q.seq < upper));
		}
		/** 问答条目的预览文本：取 `arguments` JSON 里的问题标题（多问用 ` / ` 连）。 */
		function questionsTextOf(argsRaw) {
			...
			for (const q of list) {
				...
				const o = q;
				const text = typeof o.question === "string" && o.question !== "" ? o.question : typeof o.header === "string" ? o.header : "";
		...
		*  falling back to the projection preview when the node is not mounted. */
		function fullTextOf(m, nodeSnapshot) {
			const content = chatNodeOf(nodeSnapshot, anchorKeyOf(m))?.data?.content;
		...
		*  loaded chat node's content blocks. */
		function tipImagesOf(m, nodeSnapshot) {
			...
			return imageSpecsOfContent(chatNodeOf(nodeSnapshot, anchorKeyOf(m))?.data?.content);
		}
		...
		*/
		async function jumpToMessage(sessionsService, sessionId, key, nodeOf, onProgress, signal, targetSeq, onJump) {
			const session = sessionsService.binding(sessionId)?.session;
			if (session === void 0) return false;
			const isLoaded = nodeOf === void 0 ? (k) => session.getSnapshot().chat?.nodes?.get(k) !== void 0 : nodeOf;
			let pages = 0;
			let loaded = false;
			const jumpSession = session;
			if (typeof jumpSession.loadThrough === "function" && targetSeq !== void 0 && Number.isSafeInteger(targetSeq)) {
				if (!isLoaded(key)) {
					let spin = 0;
					while (spin++ < 120) {
						if (signal?.aborted) return false;
						if (session.getSnapshot()?.loadingOlder !== true) break;
						await delay(50);
					}
					if (signal?.aborted) return false;
					onJump?.("paging");
			...
			const scrollToRow = (instant = false) => {
				const viewRect = scrollport.getBoundingClientRect();
				const rowRect = row.getBoundingClientRect();
				const target = Math.max(0, scrollport.scrollTop + (rowRect.top - viewRect.top) - (viewRect.height - rowRect.height) / 2);
				scrollport.scrollTo({
					top: target,
			...
			for (let attempt = 0; attempt < 4; attempt++) {
				await delay(600);
				if (signal?.aborted) return false;
				if (rowDelta() <= Math.max(120, scrollport.getBoundingClientRect().height * .2)) break;
				scrollToRow(true);
			}
			return true;
		}
		...
		*  placeholder while loading, and removes itself on failure. */
		...
		function TimelineRail({ useProjection, sessionId, sessionsService, chatOf, inputActions, conversation }) {
			const t = langStrings();
			const projected = useProjection("chatRail");
			const session = sessionId === void 0 ? void 0 : sessionsService.binding(sessionId)?.session;
			const fallbackStore = (sessionId === void 0 ? void 0 : chatOf?.(sessionId)) ?? (session === void 0 ? NOOP_STORE : session);
			...
			const showOfficial = (0, react.useSyncExternalStore)(railSettingsScope !== null ? (cb) => railSettingsScope.subscribe(cb) : NOOP_SUB, () => railSettingsScope?.getSnapshot() ?? { value: void 0 }).value?.showOfficialNavigator === true;
			const applyOfficialMode = (on) => {
				syncOfficialHide(on);
				if (on) {
					clearRowActions();
					if (navRef.current !== null) navRef.current.style.display = "none";
				} else if (navRef.current !== null) navRef.current.style.display = "";
			};
			(0, react.useEffect)(() => {
				applyOfficialMode(showOfficial);
			}, [showOfficial]);
			...
			*  stable, so the tip is positioned correctly). */
			...
			const tipCloseTimerRef = (0, react.useRef)(null);
			...
			(0, react.useEffect)(() => cancelTipClose, []);
			(0, react.useEffect)(() => {
				if (!show) {
			...
			(0, react.useEffect)(() => {
				...
				if (typeof document !== "undefined") return startActionInjector(t);
			}, [showOfficial, t]);
			(0, react.useEffect)(() => {
			...
			const items = effectiveMessages.map((m, i) => {
				...
				return (0, react.createElement)("button", {
					...
					...key !== void 0 ? { "data-crl-anchor": key } : {},
					...
					onClick: () => {
						...
						jumpToMessage(sessionsService, sessionId, nodeKeyOf(m), (k) => chatNodeOf(fallbackStore.getSnapshot(), k), void 0, controller.signal, m.seq, (phase) => setJumping(phase === "paging")).finally(() => setJumping(false));
					},
			...
			return (0, react_dom.createPortal)([(0, react.createElement)("div", {
				...
				if (qaForTip.length > 0) children.push((0, react.createElement)("div", {
					className: S.tipQaWrap,
					key: "qa"
				}, qaForTip.map((q) => (0, react.createElement)("button", {
					type: "button",
					key: q.key ?? q.anchor,
					className: S.tipQa,
					title: q.text,
					onClick: (e) => {
						...
						const controller = new AbortController();
						jumpAbortRef.current = controller;
						jumpToMessage(sessionsService, sessionId, nodeKeyOf(q), (k) => chatNodeOf(fallbackStore.getSnapshot(), k), void 0, controller.signal, q.seq, (phase) => setJumping(phase === "paging")).finally(() => setJumping(false));
					},
					children: [(0, react.createElement)("svg", {
						key: "icon",
		...
		function apply(ctx) {
			...
			ctx.inject(["settingsScope"], (scope) => {
				bindRailSettingsScope(scope.settingsScope);
			});
			ctx.slots.inject("settings.plugin.item", () => ctx.slots.register({
				name: "settings.plugin.item",
				key: "chat-rail",
				inject: () => ({
					setShowOfficialNavigator: (show) => railSettingsScope?.set("showOfficialNavigator", show),
					t: langStrings()
				})
			}, ChatRailSettingsRow));
		}
		function ChatRailSettingsRow({ setShowOfficialNavigator, t }) {
			const [open, setOpen] = (0, react.useState)(false);
			const on = (0, react.useSyncExternalStore)(railSettingsScope !== null ? (cb) => railSettingsScope.subscribe(cb) : NOOP_SUB, () => railSettingsScope?.getSnapshot() ?? { value: void 0 }).value?.showOfficialNavigator === true;
			return (0, react.createElement)("li", { className: "crlSetCard" + (open ? " crlSetCardOpen" : "") }, (0, react.createElement)("button", {
				type: "button",
				className: "crlSetHeader",
				"aria-expanded": open,
				onClick: () => setOpen((v) => !v),
				children: [(0, react.createElement)("span", { className: "crlSetHeadText" }, (0, react.createElement)("span", { className: "crlSetName" }, t.cardName), (0, react.createElement)("span", { className: "crlSetDesc" }, t.cardDesc)), (0, react.createElement)("svg", {
				...
				className: "crlSetBody",
				children: [(0, react.createElement)("div", { className: "crlSetRow" }, [(0, react.createElement)("div", { className: "crlSetRowLabel" }, (0, react.createElement)("span", { className: "crlSetHint" }, t.officialNavHint)), (0, react.createElement)("button", {
					type: "button",
					role: "switch",
					"aria-checked": on,
					className: "crlSetSwitch" + (on ? " on" : ""),
					onClick: () => setShowOfficialNavigator(!on),
					children: (0, react.createElement)("span", { className: "crlSetKnob" })
				})])]
			}) : null);
		}
		//#endregion
		exports.ChatRailSettingsRow = ChatRailSettingsRow;
		exports.TimelineRail = TimelineRail;
		exports.apply = apply;
		exports.bindRailSettingsScope = bindRailSettingsScope;
		exports.chatNodeOf = chatNodeOf;
		exports.clearRowActions = clearRowActions;
		exports.collectFromNodes = collectFromNodes;
		exports.collectQaFromNodes = collectQaFromNodes;
		exports.favoriteIdsOf = favoriteIdsOf;
		exports.imageSpecsOfContent = imageSpecsOfContent;
		exports.inject = inject;
		exports.isFavorite = isFavorite;
		exports.jumpToMessage = jumpToMessage;
		exports.messageIdOfAnchorKey = messageIdOfAnchorKey;
		exports.normalize = normalize;
		exports.qaWithinMessage = qaWithinMessage;
		exports.railMessageOfNode = railMessageOfNode;
		exports.readFavorites = readFavorites;
		exports.syncOfficialHide = syncOfficialHide;
