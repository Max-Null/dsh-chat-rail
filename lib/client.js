window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function jumpToMessage(sessionsService, sessionId, key, nodeOf, onProgress, signal, targetSeq) {
			...
			let pages = 0;
			let loaded = false;
			const jumpLoadThrough = session.loadThrough;
			if (typeof jumpLoadThrough === "function" && targetSeq !== void 0 && Number.isSafeInteger(targetSeq)) {
				let spin = 0;
				while (spin++ < 120) {
			...
			const items = effectiveMessages.map((m, i) => {
				...
				return (0, react.createElement)("button", {
					...
					onClick: () => {
						...
						setJumping(true);
						jumpToMessage(sessionsService, sessionId, key, (k) => chatNodeOf(fallbackStore.getSnapshot(), k), void 0, controller.signal, m.seq).finally(() => setJumping(false));
					},
